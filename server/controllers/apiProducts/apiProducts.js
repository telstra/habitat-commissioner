...
router.use('/', verifyJWT, validateOrg, (req, res, next) => {
  res.locals.apiEndpoint = `${res.locals.org}/apiproducts`;
  res.locals.repoExtension = `/config/org/${res.locals.org}/apiProducts.json`;
  next();
});
...
router.get('/apigee/list', validateHostName, async (req, res, next) => {
  try {
    ...
    responseHelper.handleResponse(res, `API products from apigee`, data);
  }
  catch (e) {
...
router.get('/apigee/details/:name', validateHostName, async (req, res, next) => {
  try {
    ...
    responseHelper.handleResponse(res, `Details for API product: ${req.params.name}`, data);
  }
  catch (e) {
    responseHelper.handleError(res, e);
  }
...
router.get('/repo/list', validateRepo, async (req, res, next) => {
  try {
    responseHelper.handleResponse(res, `API products from repo`, await fileHelper.read(res.locals.repoFilePath));
  }
  catch (e) {
    responseHelper.handleError(res, e);
  }
...
router.get('/repo/details/:name', validateRepo, async (req, res, next) => {
  try {
    ...
    responseHelper.handleResponse(res, `API product ${req.params.name} from repo`, data[data.findIndex(x => x.name === req.params.name)] || { error: "Not found" });
  }
  catch (e) {
    responseHelper.handleError(res, e);
  }
...
router.post('/apigee', validateHostName, validateRepo, validateBody, validateEntryExists, async (req, res, next) => {
  try {
    ...
    responseHelper.handleResponse(res, `Finished importing API products`, results);
  }
  catch (e) {
    responseHelper.handleError(res, e);
  }
...
router.put('/apigee', validateHostName, validateRepo, validateBody, validateEntryExists, async (req, res, next) => {
  try {
    ...
    responseHelper.handleResponse(res, `Finished updating API products`, results);
  }
  catch (e) {
    responseHelper.handleError(res, e);
  }
...
router.delete('/:name', validateHostName,async (req, res, next) => {
  try {
    ...
    responseHelper.handleResponse(res, `Delete API product ${req.params.name}`, result);
  }
  catch (e) {
    responseHelper.handleError(res, e);
  }
